...
#         #     bias=False,
#         # )
#         self.linear1 = torch.nn.Linear(hidden_size, hidden_size//2, bias=True)
#         self.activation = torch.nn.SiLU()
#         self.linear2 = torch.nn.Linear(hidden_size//2, 5, bias=True)
#         # self.linear3 = torch.nn.Linear(hidden_size//2, 5, bias=True)
#         # self.linear2 = RowParallelLinear(
#         #     hidden_size,
#         #     5,  # 5-class classification: 0, 1, 2, 3, 4
#         #     bias=False,
#         # )
...
#         # hidden_states = self.activation(hidden_states)
#         # output = self.linear3(hidden_states)
#         return output
...
class Qwen3NextForCausalLM(nn.Module, HasInnerState, SupportsLoRA, SupportsPP,
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        loader = AutoWeightsLoader(
            self,
            # NOTE: `mtp_stop_head` lives in the MTP model (`qwen3_next_mtp.Qwen3NextMTP`),
            # not in the base `Qwen3NextForCausalLM`. When loading a checkpoint that
            # contains MTP-related weights, skip them here to avoid hard failure.
            skip_prefixes=["mtp.", "mtp_stop_head."],
        )
...
#         self.num_routed_experts = example_layer.n_routed_experts
#         self.num_redundant_experts = example_layer.n_redundant_experts
#         self.mtp_stop_head = MTPStopHead(self.config.hidden_size)
#     def set_eplb_state(
#         self,
#         expert_load_view: torch.Tensor,
#         logical_to_physical_map: torch.Tensor,
